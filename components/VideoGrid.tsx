  ...
  return (
    <div className={`relative group bg-black rounded-xl overflow-hidden shadow-lg border ${peer.isLocal ? 'border-slate-700' : 'border-slate-800'} flex items-center justify-center ${className || 'aspect-video'}`}>
      <video
        ...
        playsInline
        muted={peer.isLocal}
        className={`w-full h-full ${isPinned ? 'object-contain' : 'object-cover'} transition-all duration-500 scale-x-[-1] ${isBlurred && peer.isLocal ? 'blur-xl scale-110' : ''} ${peer.videoOff ? 'hidden' : 'block'}`}
      />
